...
// 1. كلاس التمهيد الأساسي لربط البلجن بالتطبيق
@CloudstreamPlugin
...
class MyPlugin : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        // التصحيح: تمرير القائمة النظيفة مباشرة
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val episodes = doc.select(".episode-link").mapNotNull {
            ...
            val epName = it.text()
            if (epUrl.isNotBlank()) {
                // التصحيح: إنشاء حلقة الحلقة وتمرير الرابط كـ Data داخلي
                newEpisode(epUrl) {
                    this.name = epName
                }
            } else null
        ...
        return if (episodes.isNotEmpty()) {
            ...
            // التصحيح: تمرير رابط الفيلم بصيغة صحيحة متوافقة مع الـ API الجديد
